...
class MagicWand:
	...
	def extract_similar_color(self, seed_point, radius=5, threshold=30):
		...
		# 色の範囲を設定
		color_range = np.array([seed_color[0] - threshold, seed_color[1] - threshold, seed_color[2] - threshold]), np.array([seed_color[0] + threshold, seed_color[1] + threshold, seed_color[2] + threshold])
	...
	def return_color_range_area(self):
		mask = cv2.inRange(self.img, *self.color_range)
